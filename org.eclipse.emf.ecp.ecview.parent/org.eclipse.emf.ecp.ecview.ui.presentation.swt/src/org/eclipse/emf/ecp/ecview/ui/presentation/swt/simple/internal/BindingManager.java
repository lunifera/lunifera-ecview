...
public class BindingManager extends
	...
	public void bindVisible(YVisibleable yVisibleAble, IRidget ridget) {
		// TODO: JCD -- IRidget.PROPERTY_SHOWING not working. Therefore send bug to Riena.
		IObservableValue uiObservable = BeansObservables
				.observeValue(ridget, "visible");
	...
	public void bindEnabled(YEnable yEnable, IRidget ridget) {
		IObservableValue uiObservable = BeansObservables
				.observeValue(ridget, IRidget.PROPERTY_ENABLED);
	...
	public void bindReadonly(YEditable yEditable, IMarkableRidget ridget) {
		// TODO: JCD -- IRidget.PROPERTY_OUTPUT_ONLY not working. Therefore send bug to Riena.
		IObservableValue uiObservable = BeansObservables
				.observeValue(ridget, "outputOnly");
		IObservableValue modelObservable = EMFObservables.observeValue(
	...
	public void bindGrouping(YNumericField yNumericField,
		...
		IObservableValue modelObservable = EMFProperties.value(FeaturePath.fromList(new EStructuralFeature[]{pckg.getYNumericField_Datatype(),extDtPckg.getYNumericDatatype_Grouping()})).observeDetail(
				masterObservable);
		IObservableValue uiObservable = BeansObservables
				.observeValue(ridget, "grouping");
		getDatabindingContext().bindValue(uiObservable, modelObservable);
	}
	...
	public void bindMarkNegative(YNumericField yNumericField,
		...
		IObservableValue masterObservable = new WritableValue();
		masterObservable.setValue(yNumericField);
		IObservableValue modelObservable = EMFProperties.value(FeaturePath.fromList(new EStructuralFeature[]{pckg.getYNumericField_Datatype(),extDtPckg.getYNumericDatatype_MarkNegative()})).observeDetail(
				masterObservable);
		IObservableValue uiObservable = BeansObservables
				.observeValue(ridget, "markNegative");
		getDatabindingContext().bindValue(uiObservable, modelObservable);
	}
